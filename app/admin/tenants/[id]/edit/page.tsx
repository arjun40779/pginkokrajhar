...
export default function EditTenantPage({ params }: { params: { id: string } }) {
  ...
  const fetchTenantAndRooms = async () => {
    try {
      setLoading(true);

      // Fetch tenant details and all rooms in parallel
      const [tenantResponse, roomsResponse] = await Promise.all([
      ...
      if (tenantResponse.ok) {
        ...
        setFormData({
          ...
          emergencyContactRelation: tenantData.emergencyContact?.relation || '',
          ...
          rentStatus: tenantData.rentStatus || 'PENDING',
  ...
  const handleInputChange = (
    ...
    // Auto-fill rent amount when room is selected (only if different from current room)
    if (name === 'roomId' && value && value !== tenant?.roomId) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (error.details) {
          ...
          setErrors({ general: error.error || 'Failed to update tenant' });
        }
      }
    } catch (error) {
      console.error('Error updating tenant:', error);
      setErrors({ general: 'Failed to update tenant. Please try again.' });
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Uncheck to mark tenant as inactive (moved out)
                </p>
              </div>
            </div>
          </div>
